...
const sendReply = async (submission) => {
  try {
    ...
    const response = await fetch('/api/send-reply', { // ← CHANGE THIS LINE
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-3 sm:p-4 md:p-6">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {selectedSubmission && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-3 sm:p-4 z-50">
            <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md mx-2">
              ...
              <p className="text-sm text-gray-600 mb-2">Email: {selectedSubmission.email}</p>
              <textarea
                value={replyMessage}
        ...
        {selectedImage && (
          <div 
            className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-3 sm:p-4 z-50"
            onClick={() => setSelectedImage(null)}
          >
            <div className="relative max-w-4xl max-h-full w-full">
              <button
                onClick={() => setSelectedImage(null)}
                className="absolute -top-10 sm:-top-12 right-0 text-white hover:text-gray-300 text-xl sm:text-2xl z-10 bg-black bg-opacity-50 rounded-full w-8 h-8 flex items-center justify-center"
              >
                ✕
              </button>
              <img
        ...
        <div className="grid gap-3 sm:gap-4">
          {submissions.map((submission) => (
            <div key={submission._id} className="bg-white rounded-lg p-4 sm:p-6 border shadow-sm">
              <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                {/* Submission Details */}
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-lg text-gray-900 break-words">{submission.name}</h3>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3">
                    <div>
                      <p className="text-gray-700 text-sm sm:text-base">
                        <strong className="text-gray-900">Address:</strong> 
                        <span className="break-words ml-1">{submission.address}</span>
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-700 text-sm sm:text-base">
                        <strong className="text-gray-900">Phone:</strong> 
                        <span className="break-words ml-1">{submission.phone}</span>
                      </p>
                    </div>
                    <div className="sm:col-span-2">
                      <p className="text-gray-700 text-sm sm:text-base break-words">
                        <strong className="text-gray-900">Email:</strong> {submission.email}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-700 text-sm sm:text-base">
                        <strong className="text-gray-900">Condition:</strong> {submission.condition}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-700 text-sm sm:text-base">
                        <strong className="text-gray-900">Desired Price:</strong> {submission.desiredAmount}
                      </p>
                    </div>
                  </div>

                  <p className="text-gray-700 mt-3 text-sm sm:text-base">
                    <strong className="text-gray-900">Reason:</strong> {submission.reason}
                  </p>
                        ...
                        {submission.images.map((imageUrl, index) => (
                          ...
                          >
                            <img
                              ...
                              className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 object-cover rounded-lg border-2 border-gray-200 shadow-sm hover:border-blue-400 transition-all"
                              onError={(e) => {
                ...
                <div className="flex flex-row lg:flex-col gap-2 justify-between lg:justify-start lg:min-w-[140px]">
                  ...
                  <span className={`px-2 py-1 text-xs text-center rounded-full font-medium flex items-center justify-center flex-1 lg:flex-none
                    ${submission.status === 'successful' ? 'bg-green-100 text-green-800 border border-green-200' : 
                      submission.status === 'rejected' ? 'bg-red-100 text-red-800 border border-red-200' : 
                      submission.status === 'contacted' ? 'bg-blue-100 text-blue-800 border border-blue-200' :
                      submission.status === 'offered' ? 'bg-purple-100 text-purple-800 border border-purple-200' :
